...
// Generated by CoffeeScript 1.9.3
(function() {
  var api, container, div, g, h4, h6, img, pageviews, randomNum, ref, secret, span, svg, username;
  ...
  Realtime.Components.pageViewTotals = React.createClass({
    ...
    runAjax: function() {
      return (function(_this) {
        return function() {
          var arr, j, len, node, page, pages;
          arr = [];
          pages = ['OP1', 'OP2', 'Confirmation'];
          for (j = 0, len = pages.length; j < len; j++) {
            page = pages[j];
            if (page === 'OP1') {
              arr.push({
                name: 'OP1',
                value: randomNum(7500, 10000)
              });
            } else if (page === 'OP2') {
              arr.push({
                name: 'OP2',
                value: randomNum(5000, 7000)
              });
            } else if (page === 'Confirmation') {
              arr.push({
                name: 'Confirmation',
                value: randomNum(2500, 4000)
              });
            }
          }
          if (_this.isMounted()) {
            ...
            return _this.d3BarChart(node, arr, _this.state);
          }
        };
      })(this)();
    },
    increment: function() {
      ...
      for (j = 0, len = ref1.length; j < len; j++) {
        page = ref1[j];
        page.value += Math.floor(Math.random() * 10) + 1;
      }
      if (this.isMounted()) {
        node = React.findDOMNode(this.refs.topBarChart);
        return this.d3BarChart(node, this.state.data, this.state);
      }
    },
    componentDidMount: function() {
      this.runAjax();
    ...
    d3BarChart: function(node, data, state) {
      ...
      percLabels.text(function(d) {
        if (d.name === 'Confirmation') {
          return percent(d.value / data[1].value);
        } else {
          return percent(d.value / data[0].value);
        }
      }).attr('x', function(d, i) {
        return x(i) + (x.rangeBand() / 2);
      }).attr('y', function(d, i) {
